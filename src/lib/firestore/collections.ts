...
export const matchDoc = (matchId: string): DocumentReference<Match> =>
  doc(collection(getDb(), 'matches'), matchId).withConverter(
    matchConverter
  ) as DocumentReference<Match>

export const betDoc = (matchId: string, betId: string): DocumentReference<Bet> =>
  doc(collection(getDb(), `matches/${matchId}/bets`), betId).withConverter(
    betConverter
...
export const participantDoc = (
  ...
  doc(collection(getDb(), `matches/${matchId}/participants`), participantId).withConverter(
    participantConverter
...
export const ledgerEntryDoc = (matchId: string, entryId: string): DocumentReference<LedgerEntry> =>
  doc(collection(getDb(), `matches/${matchId}/ledger`), entryId).withConverter(
    ledgerConverter
  ) as DocumentReference<LedgerEntry>

export const auditEntryDoc = (matchId: string, auditId: string): DocumentReference<AuditEntry> =>
